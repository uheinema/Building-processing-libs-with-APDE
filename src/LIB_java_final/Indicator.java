package my.cool.stuff;

import processing.core.*;

// here come the classes which will become a librsry
// overstructured, but as an example


abstract
  public class Indicator implements FloatAdapter<Indicator> { // almost an interface
  float value=0;
  
  public Indicator set(float f) {
    setraw(f);
    return this;
  }

  private void setraw(float f) {
    value=f;
  }

  public float get() {
    return value;
  }

  int x=10, y=10;
  float w=500, h=100;
  float mini=0, maxi=1;

  public Indicator box(float xx, float yy, float ww, float hh) {
    x=PApplet.parseInt(xx);
    y=PApplet.parseInt(yy);
    w=ww;
    h=hh;
    return this;
  }

  public Indicator range(float mi, float ma) {
    mini=mi;
    maxi=ma;
    return this;
  }

  protected float u() {
    return PApplet.map(value, 1.0f*mini, maxi, 0, 1.0f);
  }

  abstract public void draw(PGraphics g);
}


